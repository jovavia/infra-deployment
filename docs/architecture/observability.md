...
**The PostgreSQL dashboard shows "No data" on every panel.** This was a known issue with the previous community-imported dashboard (Grafana.com ID `9628`), whose `instance` template variable chained through `release` and `kubernetes_namespace` labels that only exist when `postgres_exporter` is deployed via the prometheus-community Helm chart — labels a plain Docker Compose deployment never produces. That dashboard has been replaced by `jovavia-postgres-overview.json`, whose template variables are built directly against this stack's `static_configs`-based metric labels (e.g. `label_values(pg_up, instance)`), so this failure mode no longer applies. If you still see "No data," treat it as an actual scrape or query problem — check `pg_up` directly in Prometheus's `/graph` UI before assuming a dashboard-variable issue.
